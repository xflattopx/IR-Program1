...
#writes character frequency to output file
#out_file is the file in which we will be writing to.
def write_to_outfile(out_file):
    ofile = open(out_file,'w') #writes to the file
    i = 0
    while i < len(charFreq):
		ofile.write(chr(i + 97) + "-" + str(charFreq[i]) + "\n")
		print(chr(i + 97) + "-" +  str(charFreq[i]) + "\n")
		i += 1
...
write_to_outfile('char_freq.txt')
